...
import {
  StickyNote, Plus, Search, Pencil, Trash2, X, Loader2, Inbox, Tag,
} from "lucide-react";
...
export default function Notes() {
  ...
  return (
    <motion.div
      ...
      className="p-4 lg:p-6 max-w-5xl mx-auto space-y-5"
    >
      {/* Header */}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    ...
                    className="w-6 h-6 rounded-md flex items-center justify-center text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-destructive hover:bg-destructive/5 transition-all flex-shrink-0"
                  >
                    <Trash2 className="w-3 h-3" />
                  </button>
                </div>
                <p className="text-xs text-muted-foreground line-clamp-3 mt-1.5 min-h-[3rem]">
                  {note.content || "No content"}
                </p>
                <div className="flex items-center justify-between mt-3 pt-2 border-t border-border">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${categoryColors[note.category] || categoryColors.general}`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <input
          ...
          className="w-full px-3.5 py-2.5 rounded-xl bg-muted/40 border border-border text-sm text-foreground placeholder:text-muted-foreground/60 outline-none focus:ring-2 focus:ring-ring/20"
          autoFocus
        />
